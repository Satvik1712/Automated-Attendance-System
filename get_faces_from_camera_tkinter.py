...
class Face_Register:
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char:
            existing_folders = os.listdir(self.path_photos_from_camera)
            for folder in existing_folders:
                if folder.endswith(self.input_name_char):
                    self.current_face_dir = self.path_photos_from_camera + folder
                    self.log_all["text"] = "Using existing folder: " + self.current_face_dir
                    logging.info("Using existing folder: %s", self.current_face_dir)
                    self.face_folder_created_flag = True  # Face folder already created
                    return

        # If the input name doesn't exist, create a new folder
    ...
    def save_current_face(self):
        if self.face_folder_created_flag:
            ...
            # No face detected in the current frame and folder already exists
